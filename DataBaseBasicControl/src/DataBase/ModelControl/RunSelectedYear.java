...
import DataBase.Connection.Result.ReadTable.SelectedYear.ReadSelectedYear;
import DataBase.Property.PropertyFile;
import DataBase.Setting.BasicSetting;

public class RunSelectedYear {
	private ArrayList<String[]> dataResult = new ArrayList<String[]>();

	public RunSelectedYear(TreeMap<String, String> profile)
			throws SQLException, ClassNotFoundException, ParseException {

		// selectYear[0] = model not the year
		String selectYear[] = profile.get("model").trim().split(",");
		...
		// when more than one selected year
		for (int i = 1; i < selectYear.length; i++) {
			readImplement.setSelectYear(selectYear[i]);

			// get the resultSet and the function which included
			ReadSelectedYear readDB = new ReadSelectedYear(readImplement);
