	...
	public PropertyFile(String fileAdd) throws IOException {
		...
		for (int i = 3; i < content.length; i++) {
			if (content[i].contains("Section")) {
				int order = Integer.parseInt(content[i].split("Section")[1].trim());
				...
				for (int j = i; j < i + dataBaseProfile; j++) {
					...
					} else if (content[j].contains("readIP")) {
					...
					} else if (content[j].contains("insertIP")) {
